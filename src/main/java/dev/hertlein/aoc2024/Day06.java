...
public class Day06 implements Day {
            ...
            for (int y = 0; y < inputLines.size(); y++) {
                for (int x = 0; x < inputLines.get(y).length(); x++) {
                    ...
                    if (symbol.isObstruction()) {
                        obstructions.add(new Coordinate(y, x));
                    } else if (symbol.isGuard()) {
                        this.guard = new Guard(new GuardPosition(new Coordinate(y, x), symbol.toGuardDirection()));
                    }
                }
            }
            this.obstructions = Set.copyOf(obstructions);
    ...
    private enum Direction {
        ...
        Coordinate forwardFrom(Coordinate currentPosition) {
            return switch (this) {
                case NORTH -> new Coordinate(currentPosition.y - 1, currentPosition.x);
                case SOUTH -> new Coordinate(currentPosition.y + 1, currentPosition.x);
                case WEST -> new Coordinate(currentPosition.y, currentPosition.x - 1);
                case EAST -> new Coordinate(currentPosition.y, currentPosition.x + 1);
            };
        }
    }
}
